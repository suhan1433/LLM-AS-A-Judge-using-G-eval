...
이 방법론은 검색 성능 파이프라인 구축 과정에서 Question과 Answer만 주어진 상황에서 골든 청크(Golden Chunk)를 선정하기 위해 개발되었습니다. 
기존 LLM-as-a-judge 방식의 한계를 보완하여 모델의 판단 불확실성을 정량적으로 반영한 평가 시스템을 구현했습니다
...
기존 평가 도구의 제약
DeepEval 등 기존 평가 프레임워크는 다음과 같은 한계가 있습니다:
...
각 점수 토큰의 확률 분포 추출
확률 가중 평균을 통한 최종 점수 계산
모델 불확실성을 정량적으로 반영한 보다 신뢰할 수 있는 평가 결과 제공

이를 통해 단순한 점수 출력을 넘어서 모델의 판단 과정과 확신도까지 고려한 정밀한 LLM-as-a-judge 평가 시스템을 구현했습니다.
