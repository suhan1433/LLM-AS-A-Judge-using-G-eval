...
이떄 점수를 얻기 위해 토큰의 확률 값을 활용하여 최종 점수를 주어 모델의 불확실성을 반영되도록 하여 좀 더 


## 기존 LLM-as-a-judge의 문제점

확률 값을 활용하지 않을 경우 문제점

모델의 최종 생성 점수(예: “4”)만으로 평가할 경우, 모델의 불확실성이 반영되지 않는 문제가 발생함.
이를 보완하기 위해, 모델이 다른 점수 후보에 얼마나 가능성을 두었는지까지 평가에 반영.

e.g. “4”의 확률이 0.62, “3”의 확률이 0.38인 경우, 확신이 낮은 판단이지만, 기존 방법은 “4”만을 생성하여 문제가 됨.
따라서, 각 점수 토큰의 확률 분포를 반영하여 보다 정밀한 평가를 수행함.

DeepEval 같은 평가 프레임워크를 사용할 때, 원하는 토큰을 통해 평가하는지와 확률 분포를 확인 할 수 없어 직접 OpenAI API의 토큰의 LogProb을 뽑아 구하도록 했습니다.
